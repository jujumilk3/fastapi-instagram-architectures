...
from ddd.infrastructure.api.dependencies import (
    get_auth_service,
    get_comment_service,
    get_current_user_id,
    get_feed_service,
...
from ddd.infrastructure.api.schemas import (
    CommentCreate,
    CommentResponse,
...
def _user_to_response(u: UserAggregate) -> UserResponse:
    return UserResponse(
        id=u.id,
...
async def register(
    data: UserCreate,
    svc: AuthApplicationService = Depends(get_auth_service),
):
    return _user_to_response(
        await svc.register(data.username, data.email, data.password, data.full_name)
    )
...
async def get_user(
    user_id: int,
    svc: UserApplicationService = Depends(get_user_service),
):
...
async def update_me(
    ...
    return _user_to_response(
        await svc.update_me(user_id, **data.model_dump(exclude_unset=True))
    )
...
async def get_user_posts(
    ...
    limit: int = 20,
    offset: int = 0,
    svc: PostApplicationService = Depends(get_post_service),
):
...
async def toggle_like(
    post_id: int,
    user_id: int = Depends(get_current_user_id),
    svc: LikeApplicationService = Depends(get_like_service),
):
...
async def get_comments(
    ...
    limit: int = 50,
    offset: int = 0,
    svc: CommentApplicationService = Depends(get_comment_service),
):
    return await svc.get_by_post(post_id, limit, offset)


@post_router.post(
    "/{post_id}/comments", response_model=CommentResponse, status_code=201
)
...
async def follow_user(
    following_id: int,
    user_id: int = Depends(get_current_user_id),
    svc: FollowApplicationService = Depends(get_follow_service),
):
...
async def get_feed(
    ...
    offset: int = 0,
    user_id: int = Depends(get_current_user_id),
    svc: FeedApplicationService = Depends(get_feed_service),
):
...
async def create_story(
    data: StoryCreate,
    user_id: int = Depends(get_current_user_id),
    svc: StoryApplicationService = Depends(get_story_service),
):
...
async def send_message(
    data: MessageCreate,
    user_id: int = Depends(get_current_user_id),
    svc: MessageApplicationService = Depends(get_message_service),
):
...
async def list_notifications(
    ...
    offset: int = 0,
    user_id: int = Depends(get_current_user_id),
    svc: NotificationApplicationService = Depends(get_notification_service),
):
...
async def search_users(
    q: str,
    limit: int = 20,
    svc: SearchApplicationService = Depends(get_search_service),
):
